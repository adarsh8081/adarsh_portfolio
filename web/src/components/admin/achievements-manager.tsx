...
import { Plus, Edit, Trash2, Award, FileText, Trophy, Star } from "lucide-react";
...
export function AchievementsManager({ onUpdate }: AchievementsManagerProps) {
	...
	const deleteAchievement = async (id: number) => {
		...
		try {
			...
			const response = await fetch(`http://localhost:4000/api/achievements/${id}`, {
				method: "DELETE",
				headers: {
...
function AchievementForm({
	...
	const [formData, setFormData] = useState({
		...
		category: achievement?.category || "award" as const,
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			const url = achievement
				? `http://localhost:4000/api/achievements/${achievement.id}`
				...
				<div className="grid grid-cols-2 gap-4">
					<div>
						...
						<select
							...
							onChange={(e) => setFormData({ ...formData, category: e.target.value as "award" | "certification" | "milestone" | "recognition" })}
							className="w-full px-3 py-2 rounded-lg border border-white/20 dark:border-white/10 bg-white/10 dark:bg-black/20"
						>
